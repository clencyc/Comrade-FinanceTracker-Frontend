...
const Dashboard = () => {
    ...
    const data = [
        {
            ...
            change: `$${Math.max(parseFloat(savingsGoal.target_amount) - parseFloat(savingsGoal.current_amount), 0).toFixed(2)} to goal`,
    ...
    return (
        <div className="ml-64 p-6 min-h-screen bg-gray-100 transition-all md:ml-64 sm:ml-0" onClick={() => setSelectedItem(null)}>
            ...
            <div className="grid md:grid-cols-3 gap-6 mt-4">
                {data.map((item, index) => (
                    <div key={index} className={`p-4 rounded-lg shadow-md ${item.bgColor} relative`}>
                        <div className="hover:bg-green-400 bg-gray-600 absolute top-2 right-2 text-white rounded-2xl cursor-pointer p-2" onClick={(e) => { e.stopPropagation(); toggleMenu(item); }}>
                            <MoreVertical size={18} />
                        </div>
                        {selectedItem?.title === item.title && (
                            <div className="absolute right-2 top-10 bg-white shadow-md rounded-lg w-32 p-2">
                                <ul className="text-sm text-gray-700">
                                    <li className="hover:bg-gray-200 p-2 cursor-pointer" onClick={(e) => { 
                                        e.stopPropagation();
                                        if (item.title === "Expenses") setShowExpenseListModal(true);
                                        if (item.title === savingsGoal.goal || item.title === "Saving Goal") setShowSavingsModal(true);
                                    }}>View Details</li>
                                    <li className="hover:bg-gray-200 p-2 cursor-pointer">Edit</li>
                                    <li className="hover:bg-red-200 p-2 cursor-pointer text-red-600">Delete</li>
                                </ul>
                            </div>
                        )}
            ...
            {showExpenseListModal && <ExpenseListModal expenses={expenses} setExpenses={setExpenses} onClose={closeModals} />}
            {showSavingsModal && <SavingsGoalForm onClose={closeModals} />}
        </div>
    );
};
